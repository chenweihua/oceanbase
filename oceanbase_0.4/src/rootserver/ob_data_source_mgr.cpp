...
void ObDataSourceMgr::inc_failed_count(const ObServer& server)
{
  ...
  else
  {
        ...
        it != proxy_list.end() && !found; ++it)
    {
      ...
      else if (cur->server_ == server && cur->failed_count_ < MAX_FAILED_COUNT)
      {
        ++cur->failed_count_;
        found  = true;
      }
    ...
    else
    {
      ...
      else
      {
            ...
            it != proxy_list.end() && !found; ++it)
        {
          ...
          else if (cur->server_.is_same_ip(dest_server) && cur->failed_count_ < MAX_FAILED_COUNT
      ...
      else
      {
        ...
        while (seq >= 0 && OB_SUCCESS == ret)
        {
          if (it == proxy_list.end())
          {
            ...
            {
              ret = OB_INVALID_ARGUMENT;
              TBSYS_LOG(ERROR, "no availble proxy support type %.*s", data_source_name.length(), data_source_name.ptr());
              print_data_source_info();
              break;
            }
            else
          ...
          else if (cur->failed_count_ < MAX_FAILED_COUNT && cur->is_support(data_source_name))
          {
            --seq;
            proxy = cur;
            found = true;
        ...
        else
        {
          ...
          else
          {
            ...
            for(;OB_SUCCESS == ret && it != cur_it; ++it)
            {
              ...
              if (data_source_server != cur->server_ && cur->failed_count_ < MAX_FAILED_COUNT
                  && cur->is_support(data_source_name))
              {
