...
namespace oceanbase
{
  namespace chunkserver
  {
        ...
        int64_t &sstable_type)
    {
      ...
      {
        TBSYS_LOG(ERROR, "send request to %s failed, %d ", to_cstring(src_server_), ret);
      ...
      {
        TBSYS_LOG(ERROR, "deserialize response failed, %d", ret);
      }
    ...
    int ObTabletRemoteReader::read_next(common::ObDataBuffer& buffer)
    {
      ...
      else if(has_new_data_ && OB_ITER_END == ret)
      {
        ...
        {
          TBSYS_LOG(ERROR, "send request failed, %d", ret);
          ret = OB_DATA_SOURCE_TIMEOUT;
        }
      ...
      {
        TBSYS_LOG(ERROR, "remote read failed, ret:%d", ret);
      }
    ...
    int ObTabletFileReader::prepare(const ObNewRange& range, const int64_t req_tablet_version, const int16_t remote_sstable_version)
    {
      ...
      if(OB_SUCCESS != (ret = tablet_image.acquire_tablet(range, ObMultiVersionTabletImage::SCAN_FORWARD, req_tablet_version, tablet_)))
      {
        TBSYS_LOG(ERROR, "not found the tablet range:%s tablet_version:%ld, ret:%d", to_cstring(range), req_tablet_version, ret);
      }
      else
      {
    ...
    int ObTabletFileReader::open()
    {
      ...
      if(NULL == tablet_)
      {
        TBSYS_LOG(ERROR, "not found tablet, tablet is NULL");
      }
      else
      {
