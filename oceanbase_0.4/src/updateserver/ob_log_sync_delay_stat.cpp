...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    int ObLogSyncDelayStat::add_log_replay_event(const int64_t log_id, const int64_t mutation_time, const int64_t master_log_id)
    {
      int err = OB_SUCCESS;
      ...
      else if (log_id + 1 != master_log_id) // 没追上
      {}
      else if (catchup_seq_ & 1)
      {
        catchup_seq_++;
        TBSYS_LOG(INFO, "slave catchup %ld, master_log_id=%ld", catchup_seq_>>1, master_log_id);
        set_state_as_active();
      }

      if (last_report_time_us_ + report_interval_us_ < now_us
          && n_lagged_mutator_ != n_lagged_mutator_since_last_report_
          && 1 != catchup_seq_) // 第一次启动时肯定处于not sync状态，但是这时不打印警告
      {
