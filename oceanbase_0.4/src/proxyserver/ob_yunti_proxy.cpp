...
namespace oceanbase
{
  namespace proxyserver
  {
    ...
    int YuntiProxy::open_rfile(FILE* &fp, const char* file_name)
    {
      ...
      {
        TBSYS_LOG(ERROR, "get file:%s locate path failed, error:%d error_str:%s", file_name, errno, strerror(errno));
    ...
    int YuntiProxy::fetch_data(FILE* fp, char* buf, int64_t size, int64_t &read_size)
    {
      ...
      if(!fp)
      {
        TBSYS_LOG(ERROR, "fp is null, maybe file not open");
        read_size = 0;
      }
      else if(ferror(fp))
      {
        TBSYS_LOG(ERROR, "fp read failed, errno:%d, error_str:%s", errno, strerror(errno));
