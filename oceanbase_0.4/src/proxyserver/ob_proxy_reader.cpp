...
namespace oceanbase
{
  namespace proxyserver
  {
        ...
        int64_t req_tablet_version, int16_t remote_sstable_version, const char* sstable_dir)
    {
      ...
      {
        TBSYS_LOG(ERROR, "yunti has some problem, or path:%s not exist", sstable_dir);
      }
    ...
    int YuntiFileReader::read_next(common::ObDataBuffer& buffer)
    {
      ...
      if(!has_remain_data())
      {
        ...
        {
          TBSYS_LOG(ERROR, "yunti has some problem, read_size:%ld ret:%d", read_size, ret);
          ret = OB_DATA_SOURCE_READ_ERROR;
        }
        else
        {
          if(read_size < 0)
          {
            TBSYS_LOG(ERROR, "yunti has some problem, read_size:%ld", read_size);
            ret = OB_DATA_SOURCE_READ_ERROR;
          }
          else
          {
    ...
    int YuntiRangeReader::prepare(YuntiProxy* proxy, const char* sstable_dir)
    {
      return proxy->fetch_range_list(sstable_dir, range_list_);
    }
