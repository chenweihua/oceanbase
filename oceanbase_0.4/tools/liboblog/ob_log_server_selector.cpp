  ...
  namespace liboblog
  {
    ...
    int ObLogServerSelector::get_ups(common::ObServer &server, const bool change)
    {
      ...
      else
      {
        ...
        if (change)
        {
          int64_t old_v = cur_ups_idx_;
          int64_t new_v = ((old_v + 1) >= ups_vector_.count()) ? 0 : (old_v + 1);
          ATOMIC_CAS(&cur_ups_idx_, old_v, new_v);
        }
        if (0 <= cur_ups_idx_
            && cur_ups_idx_ < ups_vector_.count())
        {
          ...
          {
            TBSYS_LOG(INFO, "get_ups addr=%s cur_ups_idx=%ld", to_cstring(server), cur_ups_idx_);
          }
        }
        else
        {
    ...
    int ObLogServerSelector::query_server_addr_()
    {
      ...
      if (NULL == (mysql = mysql_init(NULL)))
      {
        ...
        ret = OB_ERR_UNEXPECTED;
      ...
      else if (mysql != mysql_real_connect(mysql,
            ob_mysql_addr_,
            ob_mysql_user_,
            ob_mysql_password_,
            OB_MYSQL_DATABASE,
            ...
            NULL, 0))
      {
        TBSYS_LOG(WARN, "connect to ob_mysql %s:%d fail, %s", ob_mysql_addr_, ob_mysql_port_, mysql_error(mysql));
        ret = OB_ERR_UNEXPECTED;
      }
    ...
    int ObLogServerSelector::query_cluster_info_(MYSQL *mysql, IdRoleMap &cluster_id2role)
    {
      int ret = OB_SUCCESS;
      int mysql_ret = 0;
      MYSQL_RES *mysql_results = NULL;
      if (0 != (mysql_ret = mysql_query(mysql, SQL_QUERY_CLUSTER_INFO)))
      {
        TBSYS_LOG(WARN, "query sql [%s] fail, mysql_ret=%d", SQL_QUERY_CLUSTER_INFO, mysql_ret);
        ret = OB_ERR_UNEXPECTED;
      }
      else if (NULL == (mysql_results = mysql_store_result(mysql)))
      {
      ...
      else
      {
        ...
        while(NULL != (mysql_record = mysql_fetch_row(mysql_results)))
        {
    ...
    int ObLogServerSelector::query_ups_info_(MYSQL *mysql, IdRoleMap &cluster_id2role)
    {
      int ret = OB_SUCCESS;
      int mysql_ret = 0;
      MYSQL_RES *mysql_results = NULL;
      if (0 != (mysql_ret = mysql_query(mysql, SQL_QUERY_UPS_ADDR)))
      {
    ...
    int ObLogServerSelector::query_rs_info_(MYSQL *mysql, IdRoleMap &cluster_id2role)
    {
      int ret = OB_SUCCESS;
      int mysql_ret = 0;
      MYSQL_RES *mysql_results = NULL;
      if (0 != (mysql_ret = mysql_query(mysql, SQL_QUERY_RS_ADDR)))
      {
